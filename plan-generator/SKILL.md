...
核心质量线是可执行粒度：每条实施步骤落到“对象 + 动作 + 参数值”，明确核心修改文件与必要集成文件，声明阶段 checkpoint 间隔，并附可运行的离线/外部验收命令。
...
- [ ] 每个计划正文通过 `scripts/plan_state.py lint-plan`，新计划必备章节顺序为问题 → 决策 → 范围 → 环境预检 → 风险与回滚 → 实施步骤 → 完成标准；旧计划按兼容规则执行并在修改时迁移。
- [ ] 每个步骤区分核心修改文件与必要集成文件；完成标准分别记录离线验收和外部环境验收。
